...
def test___init___with_caller_supplied_credentials(no_env_vars, no_path_mock):
    ...
    assert api.username == MOCK_USERNAME
    assert api.password == MOCK_USER_API_KEY
    assert api.password != MOCK_PASSWORD
    assert api.url == MOCK_URL
